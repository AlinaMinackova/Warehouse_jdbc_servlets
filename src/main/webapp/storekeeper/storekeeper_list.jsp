...
<div class="container flex-grow-1 mt-4">
    ...
    <h2 class="text-center mb-4">СПИСОК ПЕРСОНАЛА</h2>

    <!-- Поиск -->
    <form class="d-flex mb-4" action="${pageContext.request.contextPath}/storekeeper/findAll" method="get">
    ...
    <table class="table table-striped">
        ...
        <tbody>
        <c:forEach var="storekeeper" items="${storekeepers}">
            <tr>
                <td>${storekeeper.lastName}</td>
                <td>${storekeeper.firstName}</td>
                <td>${storekeeper.middleName}</td>
                <td>${storekeeper.email}</td>
                <td>${storekeeper.birthday}</td>
                <td>
                    <a href="${pageContext.request.contextPath}/storekeeper/edit/${storekeeper.id}"
                       class="btn btn-outline-dark btn-sm me-2">РЕДАКТИРОВАТЬ</a>
                    <a href="${pageContext.request.contextPath}/storekeeper/delete/${storekeeper.id}"
